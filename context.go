...
func ConfigureModule(ctx *ConfigContext, mod any, raw json.RawMessage) error {
	...
	if !tagField.IsValid() || tagField.Kind() != reflect.String {
		return fmt.Errorf("uni: module type '%s' must have a 'Tag string' json field", typ.Name())
	}

	// 扫描带有 uni 标签的 blind box 字段
	for i := 0; i < typ.NumField(); i++ {
		field := typ.Field(i)
		uniTag := field.Tag.Get("uni")
		if uniTag == "" {
			continue
		}

		tagMap := parseUniTag(uniTag)
		namespace := tagMap["namespace"]
		if namespace == "" {
			return fmt.Errorf("uni: struct tag on field '%s.%s' requires 'namespace=...'", typ.Name(), field.Name)
		}
...
func parseUniTag(tag string) map[string]string {
